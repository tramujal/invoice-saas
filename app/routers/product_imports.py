"""Product CSV/XLSX import: preview, confirm, and template download.

Structurally identical to app/routers/customer_imports.py -- see that
module's docstring for the preview-state strategy (no server-side token/
cache; both preview and confirm independently re-parse the uploaded
bytes).
"""
...
from app.imports.products import (
    PRODUCT_FIELD_SPECS,
...
router = APIRouter(
    prefix="/organizations/{organization_id}/products/import", tags=["product-imports"]
)
...
def preview_product_import(
    ...
    enforce_rate_limit(
        [
            RateLimitCheck(
                scope="imports:products:preview:user",
                identity=user_identity(current_user.id),
                rules=IMPORT_PREVIEW_RULES,
            ),
            RateLimitCheck(
                scope="imports:products:preview:user_ip",
    ...
    try:
        mapping_result, preview = build_preview(
            parsed, PRODUCT_FIELD_SPECS, manual_mapping, process_row, IMPORT_MAX_PREVIEW_ROWS
        )
    ...
    logger.info(
        "product_import_preview: organization_id_hash=%s user_id_hash=%s file_type=%s "
...
def confirm_product_import(
    ...
    enforce_rate_limit(
        [
            RateLimitCheck(
                scope="imports:products:confirm:user",
                identity=user_identity(current_user.id),
                rules=IMPORT_CONFIRM_RULES,
            ),
            RateLimitCheck(
                scope="imports:products:confirm:user_ip",
                identity=user_ip_identity(request, current_user.id),
                rules=IMPORT_CONFIRM_RULES,
            ),
        ]
    ...
    logger.info(
        "product_import_confirm: organization_id_hash=%s user_id_hash=%s file_type=%s "
...
def download_product_import_template_csv(
    organization_id: str,
    db: Session = Depends(get_db),
    current_user: User = Depends(get_current_user),
) -> Response:
...
def download_product_import_template_xlsx(
    organization_id: str,
    db: Session = Depends(get_db),
    current_user: User = Depends(get_current_user),
) -> Response:
    ...
    return Response(
        ...
        headers={"Content-Disposition": 'attachment; filename="products-template.xlsx"'},
    )
