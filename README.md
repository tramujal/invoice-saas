...
- **Revenue Forecasting** *(deterministic — no AI; see
  ...
  "not included" state).
- **AI Financial Advisor** *(interprets, never calculates; see
  [`docs/financial_ai.md`](docs/financial_ai.md))* — a CFO-style executive
  report generated from the Financial Dashboard's and Revenue Forecasting's
  own deterministic numbers only: an executive summary, an overall health
  rating, evidence-cited observations and prioritized recommendations,
  strengths/risks/opportunities/next actions, and forecast commentary —
  strictly schema-validated (a malformed reply is retried once, then
  recorded as a failed report, never shown), generated by a background job
  and cached by a fingerprint of the underlying data, never blocking the
  rest of the dashboard from loading.
- **Google Sign-In** *(server-side OAuth 2.0/OIDC; see
  [`docs/google_auth.md`](docs/google_auth.md))* — "Continue with Google"
  on login/register, disabled by default (`GOOGLE_OAUTH_ENABLED`), never
  trusting a frontend-supplied claim: the ID token is verified server-side
  (issuer, audience, expiration, signature via Google's live JWKS, nonce,
  `email_verified`) with CSRF/replay protection on the redirect. Links
  safely to an existing password account by verified email, never
  duplicates a user, and password login/registration are completely
  unaffected whether this is enabled or not.
- **Localized notifications** *(see
  [`docs/localization.md`](docs/localization.md))* — every in-app
  notification and its corresponding email is rendered in each
  recipient's own resolved language (personal preference → organization
  default → platform default), not one shared language for the whole
  organization.
...
| `GOOGLE_OAUTH_ENABLED`, `GOOGLE_CLIENT_ID`, `GOOGLE_CLIENT_SECRET`, `GOOGLE_REDIRECT_URI` | disabled (`GOOGLE_OAUTH_ENABLED=false`) | "Continue with Google" — the app is fully unaffected (password login/registration unchanged) with these unset. See [`docs/google_auth.md`](docs/google_auth.md). |
